...
class DatabaseInfluxDB2x extends Database{
    ...
    async getRetentionPolicyForDB(dbname, callback) {
        ...
        try {
            ...
            callback(bucketData.retentionRules[0].everySeconds);
        } catch (error) {
    ...
    async applyRetentionPolicyToDB(dbname, retention, callback_error) {
        ...
        this.log.info("Applying retention policy for " + dbname + " to " + retention + " seconds. Shard Group Duration (calculated): " + shardGroupDuration + " seconds");
        try {
