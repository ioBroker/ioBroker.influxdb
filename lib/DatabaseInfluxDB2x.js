...
class DatabaseInfluxDB2x extends Database {
    constructor(log, host, port, protocol, token, organization, database, useTags, timePrecision, requestTimeout) {
        super();
    ...
    async applyRetentionPolicyToDB (dbname, retention, callback_error) {
        ...
        this.log.info("Applying retention policy for " + dbname + " to " + ((retention === 0) ? "infinity" : retention + " seconds") + ". Shard Group Duration (calculated): " + shardGroupDuration + " seconds");
        try {
    ...
    query (query, callback) {
        ...
        this.queryApi.queryRows(query, {
            ...
            error (error) {
                //Ignore errors that are related to an empty range. The handling of this currently seems inconsitent for flux.
