...
class DatabaseInfluxDB1x extends Database{
    ...
    async applyRetentionPolicyToDB(dbname, retention, callback) {
        this.getRetentionPolicyForDB(dbname, oldRetention => {
            ...
            this.log.info("Applying retention policy for " + dbname + " to " + retention + " seconds");
            this.connection.query(command + ' RETENTION POLICY "global" ON ' + dbname + ' DURATION ' + retention + 's REPLICATION 1 DEFAULT', callback);
        });
    }
